...
$emailCheckStmt = $conn->prepare("SELECT id FROM hiring_managers WHERE email = ? AND id != ?");
$emailCheckStmt->bind_param("si", $email, $id);
$emailCheckStmt->execute();
if ($emailCheckStmt->get_result()->num_rows > 0) {
    echo json_encode(["success" => false, "message" => "A hiring manager account with that email already exists."]);
...
if ($id === 0) {
    ...
    $stmt = $conn->prepare("INSERT INTO hiring_managers (first_name, last_name, email, password_hash, department) VALUES (?, ?, ?, ?, ?)");
    ...
    if ($password !== '') {
        ...
        $passwordHash = password_hash($password, PASSWORD_DEFAULT);
        $stmt = $conn->prepare("UPDATE hiring_managers SET first_name = ?, last_name = ?, email = ?, department = ?, password_hash = ? WHERE id = ?");
        $stmt->bind_param("sssssi", $firstName, $lastName, $email, $department, $passwordHash, $id);
    } else {
        $stmt = $conn->prepare("UPDATE hiring_managers SET first_name = ?, last_name = ?, email = ?, department = ? WHERE id = ?");
